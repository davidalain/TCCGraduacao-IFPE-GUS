...
%Conforme a ABNT, a contagem das páginas começa na folha de rosto (a capa não é contada).
%A versão com asterisco (\frontmatter*) mantém a numeração em algarismos arábicos.
%As páginas pré-textuais são contadas, mas o número não é impresso
%(inclusive nas páginas de abertura das listas e do sumário).
\pagestyle{empty}
...
\vspace*{2cm}
    ...
    Trabalho de conclusão de curso apresentado ao \instituicao\ \campus\ como requisito parcial para obtenção do título de \titulacao\ em \curso.\\
    \ \\
...
%LISTAS (ilustrações, tabelas, abreviaturas e siglas)
\newpage\listoffigures*
\thispagestyle{empty}
\newpage\listoftables*
\thispagestyle{empty}
\setglossarystyle{abreviacoes}
\printglossary[
    type=\acronymtype,
    title={Lista de abreviaturas e siglas}
]
\thispagestyle{empty}
%SUMÁRIO (último elemento pré-textual, conforme a ABNT)
